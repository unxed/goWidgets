...
Публичный API `vgui` проектируется максимально близким к TUI-библиотеке `vtui`. Один и тот же декларативный код UI должен с минимальными изменениями работать в терминале (`vtui`), на десктопе (`vgui` Win32/GTK/Cocoa) и в браузере (`vgui` Web).
...
2. **Публичный API скрывает платформу и построен на реактивных свойствах.** Пользователь работает с `vgui.Button`, у которой свойства (`Text`, `Enabled`, `Width`, `Height`) являются реактивными объектами (`vreactive.Property[T]`).
3. **Единый Layout-движок.** Платформенные менеджеры компоновки (Sizers в GTK, AutoLayout в Cocoa) **не используются**. Ядро `vgui` само считает координаты через решатель Cassowary (библиотека `kiwi-go`) и передает бэкенду готовые абсолютные координаты `(X, Y, Width, Height)`.
...
Вынесен в отдельный пакет и используется совместно `vgui` и `vtui`:
...
* **Event Router:** Транслирует платформенные события в абстрактные (`vgui.MouseClick`, `vgui.KeyPress`).
...
* **Задача 2:** Создать скелет проекта (`core`, `vgui`, `backends/ebiten`).
* **Задача 3:** Реализовать `vgui.Button` с реактивными свойствами `Text`, `Width`, `Height`, `Enabled`.
...
* **Задача 3:** Реализовать `vgui.Button` через `puregotk`, связав реактивные свойства `Property[T]` с нативными свойствами `GtkButton`.
...
* **Задача 2:** Маппинг виджетов `vgui` на нативные DOM-элементы (`<button>`, `<input>`, `<select>`).
...
* **Задача 3:** Финальная сверка API виджетов и реактивного слоя между `vtui` и `vgui`.
